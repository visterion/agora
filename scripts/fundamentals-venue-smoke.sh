...
# Run against prod host port 8091 after deploy to the container
#
# Expected outcomes:
...
TOKEN=$(grep -E '^AGORA_AUTH_TOKENS=' /opt/app/.env | cut -d= -f2- | tr -d ' "' | cut -d, -f1)

if [ -z "$TOKEN" ]; then
  echo "ERROR: Could not extract AGORA_AUTH_TOKENS from /opt/app/.env"
  exit 1
fi
