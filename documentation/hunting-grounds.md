...
| Instrument search (`search_instruments`) | Yahoo search endpoint only — no second provider, no fallback chain | Whatever Yahoo's search index covers, across all venues (a company can appear more than once, e.g. as a US ADR and its home-market listing) | 600s (`agora.data.cache.ttl.instrument-search-seconds`, `AGORA_DATA_CACHE_TTL_INSTRUMENT_SEARCH_SECONDS`) plus a reactive cooldown (`agora.data.search.cooldown-threshold` / `AGORA_DATA_SEARCH_COOLDOWN_THRESHOLD`, default 3 consecutive failures; `agora.data.search.cooldown-ms` / `AGORA_DATA_SEARCH_COOLDOWN_MS`, default 60000) — reactive rather than a fixed min-interval, so it doesn't abort the last query of a typing burst |
...
|---|---|---|---|
| Company profile (name, industry, exchange, cap) | Finnhub (US); Yahoo `quoteSummary` `assetProfile` fallback for non-US suffixed symbols | Finnhub's covered universe (US), global for non-US via Yahoo | US cached 7d (`agora.data.cache.ttl.finnhub-profile-seconds`, `AGORA_DATA_CACHE_TTL_FINNHUB_PROFILE`) — own TTL, decoupled from `fundamentals-seconds` so a nightly scan run finds a warm cache; non-US cached 7d (`agora.data.cache.ttl.company-profile-seconds`, `AGORA_DATA_CACHE_TTL_COMPANY_PROFILE`) — degrades to an empty (non-null) profile, uncached, on Yahoo outage |
...
| Analyst estimates, recommendation trend | Finnhub (US); Yahoo `quoteSummary` `recommendationTrend` fallback for non-US suffixed symbols | Finnhub's covered universe (US), global for non-US via Yahoo | US cached 6h (`agora.data.cache.ttl.fundamentals-seconds`, `AGORA_DATA_CACHE_TTL_FUNDAMENTALS`); non-US cached 1d (`agora.data.cache.ttl.recommendation-seconds`, `AGORA_DATA_CACHE_TTL_RECOMMENDATION`) — degrades to an empty list, uncached, on Yahoo outage |
