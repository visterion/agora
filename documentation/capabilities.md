...
| `trading.saxo.order-write-min-interval-ms` | `1100` | Minimum spacing between two Saxo order writes (POST/PATCH/DELETE on `/trade/v2/orders`) on ONE connection — Saxo allows one order operation per second per session, and Agora's transport has automatic retries disabled, so a 429 is final for that call. The pacer **waits, it never throws**: there is no wait cap and no rejection, because a throw would turn a client-side queue into a broker failure, and a multi-write sequence such as the flatten rollback interleave would be cut off mid-way by the same condition that triggered it. `0` makes the interceptor a full pass-through — no spacing and no 429 block |
