...
class GetWilliamsRToolTest {
    ...
    private GetWilliamsRTool tool(List<OhlcBar> bars) {
        return new GetWilliamsRTool(svcWith(bars), 14, 260);
    }

    @Test void risingClosesWilliamsRInRange() {
        ...
        BigDecimal wr = r.output().get("williamsR").decimalValue();
        assertThat(wr.compareTo(new BigDecimal("-100"))).isGreaterThanOrEqualTo(0);
        assertThat(wr.compareTo(BigDecimal.ZERO)).isLessThanOrEqualTo(0);
    }
