...
class GetObvToolTest {
    private final ObjectMapper mapper = new ObjectMapper();

    /** Rising closes + volume 1000 each → each bar after the first adds its volume to OBV. */
    ...
    private GetObvTool tool(List<OhlcBar> bars) {
        return new GetObvTool(svcWith(bars), 260);
    }

    @Test void risingClosesObvIsCumulativeVolume() {
        // rising(5): OBV = 1000 * (5-1) = 4000
        ...
        assertThat(r.output().get("obv").decimalValue()).isEqualByComparingTo("4000");
    }

    @Test void singleBarUnavailable() {
        var r = tool(rising(1)).call(mapper.createObjectNode().put("symbol", "AAPL"));
        assertThat(r.available()).isFalse();
    }

    @Test void missingSymbolUnavailable() {
        assertThat(tool(rising(5)).call(mapper.createObjectNode()).available()).isFalse();
    }

    @Test void namespaceIsGeneral() {
        assertThat(tool(rising(5)).namespace()).isEqualTo("general");
    }
}
