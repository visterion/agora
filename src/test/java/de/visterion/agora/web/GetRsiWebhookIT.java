...
class GetRsiWebhookIT {
    ...
    static class StubProviderConfig {
        ...
        MarketDataProvider rsiStubProvider() {
            return new MarketDataProvider() {
                public String name() { return "rsi-stub"; }
                ...
                public List<OhlcBar> ohlc(String s, int d) {
                    // 30 strictly rising closes → RSI = 100 (deterministic; a real feed could not)
    ...
    @Test void getRsiOverWebhookWithBearer() {
        RestClient http = RestClient.create();
        ResponseEntity<String> resp = http.post()
                .uri("http://localhost:" + port + "/tools/get_rsi")
                .header("Authorization", "Bearer test-token")
                .contentType(MediaType.APPLICATION_JSON)
                .body("{\"symbol\":\"AAPL\"}")
                .retrieve().toEntity(String.class);
        assertThat(resp.getStatusCode().value()).isEqualTo(200);
        assertThat(resp.getBody()).contains("\"symbol\":\"AAPL\"").contains("\"available\":true").contains("\"rsi\":100");
    }

    @Test void rejectsWithoutBearer() {
        RestClient http = RestClient.create();
        try {
            http.post().uri("http://localhost:" + port + "/tools/get_rsi")
