...
@Component
public class GetCciTool implements AgoraTool {
    ...
    // Test ctor
    GetCciTool(MarketDataService service, int defaultPeriod, int fetchDays) {
        this.service = service;
        this.defaultPeriod = defaultPeriod;
        this.fetchDays = fetchDays;
    }

    public String name() { return "get_cci"; }
    public String description() { return "Commodity Channel Index (CCI) for a symbol. Default period " + defaultPeriod + "."; }
    ...
    public ToolResult call(JsonNode args) {
        ...
        if (bars.size() < period) return ToolResult.unavailable("insufficient history for cci");
        BarSeries series = Ta4jBars.toSeries(bars);
        CCIIndicator cci = new CCIIndicator(series, period);
        Num v = cci.getValue(series.getEndIndex());
        if (v.isNaN()) return ToolResult.unavailable("insufficient history for cci");

        ObjectNode out = mapper.createObjectNode();
        out.put("symbol", symbol);
        out.put("cci", Ta4jBars.toBd(v, 4));
